...
export default async function MeetingEnd({ params, searchParams }: PageProps) {
  ...
  useEffect(() => {
    if (!isInvalid && callingState !== CallingState.LEFT) {
      router.push('/');
    }
  ...
  if (!isInvalid && callingState !== CallingState.LEFT) return null;
    ...
    <div className="w-full">
      ...
      <div className="mt-6 px-4 flex flex-col items-center gap-8">
        <h1 className="text-4xl leading-[2.75rem] font-normal text-dark-gray tracking-normal">
          {isInvalid ? 'Check your meeting code' : 'You left the meeting'}
        </h1>
        {isInvalid && (
